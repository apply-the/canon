...
- `RISK`
- `ZONE`
- either one brownfield brief file, one brownfield input folder, or enough starting intent to complete one through guided intake
...
- Verify risk, zone, and at least one starting input are present before invoking Canon.
...
- If neither canonical location exists and the user did not provide an explicit input, continue guided intake or ask explicitly for the brief path.
...
- Canon command: `canon run --mode brownfield-change --risk <RISK> --zone <ZONE> [--owner <OWNER>] --input <INPUT_PATH>`
