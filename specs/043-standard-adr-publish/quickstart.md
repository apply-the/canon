...
4. Verify one new ADR file exists under `tech-docs/adr/` with a numbered `ADR-XXXX-<slug>.md` name.
...
3. Verify the normal `tech-docs/changes/...` packet is published and no new `tech-docs/adr/ADR-*.md` file appears.
...
5. Verify the packet publish output is still present and one new ADR file is added to `tech-docs/adr/`.
...
3. Verify the migration packet publishes normally and one ADR file is added to `tech-docs/adr/`.
...
1. Publish a supported run with a packet override destination:
   ...
   ```bash
   canon publish <RUN_ID> --to tech-docs/custom/output --adr
   ```

2. Verify the normal packet files land in `tech-docs/custom/output`.
3. Verify the ADR still lands in `tech-docs/adr/` and is listed in the publish summary output.