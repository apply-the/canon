...
- Supported skills may expose an optional structured `Primary Artifact Action`
  when Canon returned a Canon-backed primary artifact in `mode_result`.
...
- `Possible Actions:` remains mandatory even when a host can render chips.
- `Recommended Next Step:` remains mandatory unless Canon already returned a
  completed, self-explanatory `mode_result`; in that case skills may omit it or
  render an explicit `None` fallback.
...
- `Primary Artifact Action` must not be invented from heuristics; its `target`
  must come from Canon-backed runtime output for the active run.
