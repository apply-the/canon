...
fn skills_install_for_codex_carries_current_runtime_compatibility_reference() {
    ...
    assert!(
        installed.contains("expected_workspace_version = \"0.32.0\""),
        "skills install should carry the 0.32.0 runtime compatibility expectation"
    );
}
...
fn skills_list_returns_all_embedded_skills() {
    ...
    assert!(names.contains(&"canon-system-assessment"), "should list canon-system-assessment");
    assert!(
