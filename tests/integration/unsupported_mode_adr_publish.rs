...
fn init_repo(workspace: &TempDir) {
    ...
    git(workspace, &["add", "."]);
    git(workspace, &["commit", "-m", "seed unsupported adr repo"]);
}

fn incident_brief() -> &'static str {
...
#[test]
fn incident_publish_rejects_adr_flag_without_creating_registry_files() {
    let workspace = TempDir::new().expect("temp dir");
    init_repo(&workspace);
    fs::write(workspace.path().join("incident.md"), incident_brief()).expect("brief file");
