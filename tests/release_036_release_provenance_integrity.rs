...
#[test]
fn release_docs_and_version_surfaces_align_on_0_42_0_delivery() {
    ...
    assert!(winget_guide.contains(VERSION), "winget guide should use {VERSION} examples");
    ...
    assert!(scoop_guide.contains(VERSION), "scoop guide should use {VERSION} examples");
    ...
    assert!(
        roadmap.contains("042-visual-artifact-generation")
            && roadmap.contains("architecture-overview.md"),
        "roadmap should name the delivered 042 slice"
    );
    assert!(
    ...
    assert!(
        ...
        "roadmap should no longer list 042 as only proposed"
    );

    let changelog = fs::read_to_string(repo_root.join("CHANGELOG.md")).expect("read changelog");
    assert!(changelog.contains("## [0.42.0]"), "changelog should record the 0.42.0 release");
    assert!(
