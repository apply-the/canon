...
fn broad_surface_brief() -> &'static str {
    "# Change Brief\n\n## System Slice\n\nauth session boundary and persistence layer.\n\n## Domain Slice\n\nSession lifecycle, repository boundaries, and adjacent auth-module touchpoints.\n\n## Excluded Areas\n\n- payment settlement\n\n## Intended Change\n\nTighten the auth persistence boundary while preserving the public auth contract.\n\n## Legacy Invariants\n\n- session revocation remains eventually consistent\n- audit log ordering stays stable\n\n## Domain Invariants\n\n- revocation semantics must remain stable even if adjacent modules are touched\n- audit trails must preserve causal order across auth-module boundaries\n\n## Forbidden Normalization\n\n- Do not collapse audit-ordering quirks that operators still rely on.\n\n## Change Surface\n\n- auth service\n- session repository\n- adjacent modules\n\n## Ownership\n\n- primary owner: maintainer\n\n## Cross-Context Risks\n\n- adjacent modules may widen the change beyond the owned auth persistence boundary\n\n## Implementation Plan\n\nTighten the auth persistence boundary while preserving the public auth contract.\n\n## Sequencing\n\n1. Tighten the persistence boundary.\n2. Validate adjacent callers before rollout.\n\n## Validation Strategy\n\n- contract tests\n- invariant checks\n\n## Independent Checks\n\n- rollback rehearsal by a separate operator\n\n## Decision Record\n\nPrefer additive change over normalization to preserve operator expectations.\n\n## Decision Drivers\n\n- preserve operator expectations around revocation and audit ordering\n- keep ownership explicit when the change surface starts to approach adjacent modules\n\n## Options Considered\n\n- Option 1 tightens the auth persistence boundary while keeping adjacent modules review-only.\n- Option 2 widens the first rollout to adjacent modules for broader consistency.\n\n## Decision Evidence\n\n- existing contract checks already cover the bounded auth surface directly\n- adjacent-module involvement increases approval pressure before the preserved behavior is revalidated\n\n## Boundary Tradeoffs\n\n- touching adjacent modules may improve consistency but increases ownership and approval pressure\n\n## Recommendation\n\n- start with the bounded auth persistence slice and escalate adjacency only with explicit approval\n\n## Why Not The Others\n\n- widening the first rollout to adjacent modules would exceed the owned change boundary too early\n\n## Consequences\n\n- preserved surface remains explicit and reviewable\n\n## Unresolved Questions\n\n- do adjacent modules stay in the first rollout?\n\nOwner: maintainer\nRisk Level: bounded-impact\nZone: yellow\n"
}
