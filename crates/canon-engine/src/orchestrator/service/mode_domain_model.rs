...
impl EngineService {
    pub(super) fn run_domain_model(
        ...
        let context_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "capture domain-model context and bounded concept surface",
        ...
        let context_attempt = self.completed_attempt(
            ...
            ToolOutcome {
                ...
                summary: format!(
                    "Captured domain-model context from {} authored input(s).",
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "generate bounded domain-model packet",
            scope: input_scope.clone(),
        });
        ...
        let critique_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "critique domain-model concepts, relationships, and invariant completeness",
            scope: input_scope.clone(),
        });
        ...
        let validation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "validate domain-model packet against repository surface",
            scope: input_scope.clone(),
        });
        ...
        let artifacts = artifact_contract
            ...
            .map(|requirement| PersistedArtifact {
                ...
                contents: render_domain_model_artifact(&requirement.file_name, &context_summary),
            })
            .collect::<Vec<_>>();
        ...
        let gate_inputs = artifacts
            ...
            .collect::<Vec<_>>();
        let gates = gatekeeper::evaluate_domain_model_gates(
            &artifact_contract,
            &gate_inputs,
