...
pub(super) struct AuthoringWorkflowSpec<'a> {
    pub mode: Mode,
    pub context_summary: &'a str,
    pub generation_summary: &'a str,
    pub critique_summary: &'a str,
}

impl EngineService {
    pub(super) fn execute_authoring_workflow<F>(
        &self,
        store: &WorkspaceStore,
        request: RunRequest,
        policy_set: crate::domain::policy::PolicySet,
        spec: AuthoringWorkflowSpec<'_>,
        mut render_artifact: F,
        ...
        F: FnMut(&str, &str, &str, &str) -> String,
    {
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: spec.generation_summary,
            scope: input_scope.clone(),
        });
        ...
        let generation_output = copilot.generate(&context_summary_data);
