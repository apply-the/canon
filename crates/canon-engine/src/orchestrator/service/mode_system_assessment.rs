...
impl EngineService {
    pub(super) fn run_system_assessment(
        &self,
        ...
        let context_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "capture system assessment context and bounded repository surface",
        ...
        let context_attempt = self.completed_attempt(
            ...
            ToolOutcome {
                ...
                summary: format!(
                    "Captured system-assessment context from {} authored input(s).",
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "generate bounded system assessment packet",
            scope: input_scope.clone(),
        });
        ...
        let critique_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "critique system assessment evidence, confidence, and gap discipline",
            scope: input_scope.clone(),
        });
        ...
        let validation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "validate system assessment packet against repository surface",
            scope: input_scope.clone(),
        });
        ...
        let artifacts = artifact_contract
            ...
            .map(|requirement| PersistedArtifact {
                ...
                contents: match artifact_slug(&requirement.file_name) {
                    ...
                    _ => {
                        render_system_assessment_artifact(&requirement.file_name, &context_summary)
                    }
                },
            })
            .collect::<Vec<_>>();
        ...
        let gate_inputs = artifacts
            ...
            .collect::<Vec<_>>();
        let gates = gatekeeper::evaluate_system_assessment_gates(
            &artifact_contract,
            &gate_inputs,
