...
impl EngineService {
    pub(super) fn run_migration(
        ...
        let context_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "capture migration packet context and bounded transition surface",
        ...
        let context_attempt = self.completed_attempt(
            ...
            ToolOutcome {
                ...
                summary: format!(
                    "Captured migration context from {} authored input(s).",
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "generate bounded migration compatibility packet",
            scope: input_scope.clone(),
        });
        ...
        let critique_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "critique migration safety, fallback posture, and residual rollout risk",
            scope: input_scope.clone(),
        });
        ...
        let validation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "validate migration packet against repository surface",
            scope: input_scope.clone(),
        });
        ...
        let artifacts = artifact_contract
            ...
            .map(|requirement| PersistedArtifact {
                ...
                contents: render_migration_artifact(&requirement.file_name, &context_summary),
            })
            .collect::<Vec<_>>();
        ...
        let gate_inputs = artifacts
            ...
            .collect::<Vec<_>>();
        let gates = gatekeeper::evaluate_migration_gates(
            &artifact_contract,
            &gate_inputs,
