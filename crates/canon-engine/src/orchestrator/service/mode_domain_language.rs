...
impl EngineService {
    pub(super) fn run_domain_language(
        &self,
        ...
        let context_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "capture domain-language context and bounded vocabulary surface",
        ...
        let context_attempt = self.completed_attempt(
            ...
            ToolOutcome {
                ...
                summary: format!(
                    "Captured domain-language context from {} authored input(s).",
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "generate bounded domain-language packet",
            scope: input_scope.clone(),
        });
        ...
        let critique_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "critique domain-language findings, glossary completeness, and conflict resolution",
            scope: input_scope.clone(),
        });
        ...
        let validation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "validate domain-language packet against repository surface",
            scope: input_scope.clone(),
        });
        ...
        let artifacts = artifact_contract
            ...
            .map(|requirement| PersistedArtifact {
                ...
                contents: render_domain_language_artifact(&requirement.file_name, &context_summary),
            })
            .collect::<Vec<_>>();
        ...
        let gate_inputs = artifacts
            ...
            .collect::<Vec<_>>();
        let gates = gatekeeper::evaluate_domain_language_gates(
            &artifact_contract,
            &gate_inputs,
