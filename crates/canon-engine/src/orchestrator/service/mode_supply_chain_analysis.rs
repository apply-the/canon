...
impl EngineService {
    pub(super) fn run_supply_chain_analysis(
        &self,
        ...
        let context_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "capture supply-chain context and bounded manifest surface",
        ...
        let context_attempt = self.completed_attempt(
            ...
            ToolOutcome {
                ...
                summary: format!(
                    "Captured supply-chain-analysis context from {} authored input(s).",
        ...
        let generation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "generate bounded supply-chain analysis packet",
            scope: input_scope.clone(),
        });
        ...
        let critique_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "critique dependency findings, license posture, and coverage gaps",
            scope: input_scope.clone(),
        });
        ...
        let validation_request = self.governed_request(GovernedRequestSpec {
            ...
            summary: "validate supply-chain packet against repository manifests and bounded surface",
            scope: input_scope.clone(),
        });
        ...
        let artifacts = artifact_contract
            ...
            .map(|requirement| PersistedArtifact {
                ...
                contents: render_supply_chain_analysis_artifact(
                    &requirement.file_name,
                    &context_summary,
                ),
            })
        ...
        let gate_inputs = artifacts
            ...
            .collect::<Vec<_>>();
        let gates = gatekeeper::evaluate_supply_chain_analysis_gates(
            &artifact_contract,
            &gate_inputs,
