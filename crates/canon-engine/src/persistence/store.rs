...
use canon_adapters::{AdapterInvocation, filesystem::FilesystemAdapter};
...
use crate::domain::policy::{
    GatePolicy, PolicySet, PolicySetOverrides, RiskPolicyClass, ZonePolicy,
};
...
impl WorkspaceStore {
    ...
    pub fn load_policy_set(&self, override_root: Option<&Path>) -> Result<PolicySet, Error> {
        ...
        let _ = adapter_file.capabilities.len();
    ...
    fn load_policy_overrides(&self, override_root: &Path) -> Result<PolicySetOverrides, Error> {
        ...
        let adapter_override = if override_root.join("adapters.toml").exists() {
            {
                ...
                let _ = adapter_policy.capabilities.len();
                Some(adapter_policy.rules.block_mutation_for_red_or_systemic)
            }
        } else {
            None
        };
        ...
        Ok(PolicySetOverrides {
            ...
            block_mutation_for_red_or_systemic: adapter_override,
        })
    }
